    ...
    const fetchCampaigns = async () => {
      ...
      try {
        ...
        const displayedCampaigns = allCampaigns.slice(0, 4).map((campaign, index) => ({
          ...
          // ✅ Convert IPFS images properly
          image: campaign.image && campaign.image.startsWith("ipfs://")
            ? `https://ipfs.io/ipfs/${campaign.image.replace("ipfs://", "")}`
            : campaign.image || "https://via.placeholder.com/300", // Default image if missing
        }));
      ...
      <div className="h-screen bg-cover bg-top bg-no-repeat flex items-center justify-center">
        <div className="max-w-[1250px] w-full flex flex-col items-center pt-[200px]">
          <div className="flex flex-col items-center justify-center mb-[150px] h-64">
            <h1 className="font-bold text-blue-950 text-6xl">
              Empower Change{" "}
              ...
              <span className="text-blue-600">Fund Transparently</span>
              </span>
            </h1>
            <div className="mt-8 max-w-[850px] font-medium text-gray-700 text-2xl text-center">
              Launch campaigns, <span className="font-bold text-gray-800">
                support causes, and <span className="text-blue-500">track funds securely</span>
              </span>{" "}
